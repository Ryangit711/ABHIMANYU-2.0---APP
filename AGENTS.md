...
| **FETCH 30 — 90%+ Quality Minimum** | FETCH and WIDENET MUST return exactly 30 jobs (unless user explicitly asks for fewer or more). Every one of the 30 must score 90%+ fit across TICS pipes — pure relevancy to Aman's profile, location (Vancouver/Remote Canada), salary ($80K+), pipe fit, and PR-qualifying (TEER 0/1). If the sweep finds fewer than 30 that meet 90%+ bar, WIDENET and deepen the search across additional sources until the full 30 are found. No filler. No "close enough." Every single one must be a genuine, high-confidence match. This is a hard floor — 30 is the minimum, not the target. Quality is non-negotiable. |
