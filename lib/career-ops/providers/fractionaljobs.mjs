...
const JOBS_URL = 'https://fractionaljobs.io/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseFractionalJobsHtml(text);
  },
};

function parseFractionalJobsHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'FractionalJobs';
