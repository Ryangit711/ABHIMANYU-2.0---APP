...
const JOBS_URL = 'https://remotedom.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseRemoteDomHtml(text);
  },
};

function parseRemoteDomHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'RemoteDom';
