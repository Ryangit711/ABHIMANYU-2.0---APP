...
const JOBS_URL = 'https://ceriusexecutives.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseCeriusHtml(text);
  },
};

function parseCeriusHtml(html) {
      ...
      for (const item of items) {
        ...
        if (job && job.title) {
          jobs.push({
            ...
            url: job.url || '',
            company: (job.hiringOrganization?.name || 'Cerius Executives').trim(),
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'Cerius Executives';
