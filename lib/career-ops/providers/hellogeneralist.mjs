...
const JOBS_URL = 'https://www.hellogeneralist.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseHelloGeneralistHtml(text);
  },
};

function parseHelloGeneralistHtml(html) {
      ...
      for (const item of items) {
        ...
        if (job && job.title) {
          jobs.push({
            ...
            url: job.url || '',
            company: (job.hiringOrganization?.name || 'Hello Generalist').trim(),
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].trim() : 'Hello Generalist';
