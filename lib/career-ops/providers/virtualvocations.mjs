...
const JOBS_URL = 'https://www.virtualvocations.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseVirtualVocationsHtml(text);
  },
};

function parseVirtualVocationsHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'Virtual Vocations';
