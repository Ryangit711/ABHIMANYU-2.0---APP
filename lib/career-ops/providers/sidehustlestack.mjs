...
const JOBS_URL = 'https://sidehustlestack.co/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseSideHustleHtml(text);
  },
};

function parseSideHustleHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'Side Hustle Stack';
