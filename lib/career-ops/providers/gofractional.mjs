...
const JOBS_URL = 'https://gofractional.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseGoFractionalHtml(text);
  },
};

function parseGoFractionalHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'GoFractional';
