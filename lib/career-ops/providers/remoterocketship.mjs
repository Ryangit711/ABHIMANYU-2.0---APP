...
const JOBS_URL = 'https://remoterocketship.com/jobs';
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return parseRemoteRocketshipHtml(text);
  },
};

function parseRemoteRocketshipHtml(html) {
  ...
  for (const block of blocks) {
    ...
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'Remote Rocketship';
