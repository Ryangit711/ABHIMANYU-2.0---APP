...
const JOBS_URL = 'https://builtin.com/jobs';
...
function parseBuiltinHtml(html) {
      ...
      for (const item of items) {
        ...
        if (job && job.title) {
          jobs.push({
            ...
            url: job.url || job['@id'] || '',
            company: (job.hiringOrganization?.name || 'Built In').trim(),
  ...
  for (const block of blocks) {
    ...
    const companyMatch = block.match(/class="[^"]*company[^"]*"[^>]*>([\s\S]*?)</i) || block.match(/<span[^>]*company[^>]*>([\s\S]*?)<\/span>/i);
    const company = companyMatch ? companyMatch[1].replace(/<[^>]+>/g, '').trim() : 'Built In';

    const locationMatch = block.match(/(?:location|place)[^:]*:\s*([^<,]+)/i);
